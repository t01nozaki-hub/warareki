          ...
          {yearData.results.map((res) => (
            <div key={res.name} className={`p-5 rounded-xl border flex flex-col sm:flex-row gap-4 relative overflow-hidden transition-colors ${
              ...
              <div className="flex-shrink-0 flex flex-col items-center justify-center sm:w-20">
                ...
                {res.totalScore && (
                  <span className="text-xs font-bold text-accent mt-1 bg-accent/10 px-2 py-0.5 rounded">
                    {res.totalScore}点
                  </span>
                )}
              </div>
              ...
              <div className="flex-grow">
                ...
                {res.note && (
                  <p className="text-xs text-slate-400 flex items-center gap-1 bg-background/50 inline-block px-2 py-1 rounded">
