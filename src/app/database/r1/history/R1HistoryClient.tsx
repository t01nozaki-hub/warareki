...
export default function R1HistoryClient() {
  const searchParams = useSearchParams();
  const yearParam = searchParams.get('year');
  const defaultYear = 2025;
  const currentYear = yearParam ? parseInt(yearParam, 10) : defaultYear;
  
  const yearData = r1HistoryData.find(d => d.year === currentYear) || r1HistoryData[r1HistoryData.length - 1];
    ...
    <div className="container mx-auto px-4 py-12 max-w-6xl">
      <div className="mb-8">
        <div className="flex items-center gap-2 text-sm text-slate-400 mb-4">
          ...
          <span>&gt;</span>
          <Link href="/database/r1" className="hover:text-accent transition-colors">R-1グランプリ トップ</Link>
          <span>&gt;</span>
        ...
        <p className="text-slate-400 max-w-3xl leading-relaxed">
          第1回（2002年）から最新大会までの参加組数、審査員一覧、そして一人芸の頂点を目指した全ファイナリストの詳細を一覧できます。
        </p>
      </div>
      ...
      <div className="bg-card border border-border p-4 rounded-xl shadow-lg mb-10 overflow-x-auto">
        <div className="flex flex-nowrap gap-2 pb-2">
          {r1HistoryData.map((d) => (
            <Link 
              key={d.year} 
              href={`/database/r1/history?year=${d.year}`}
        ...
        <div className="lg:col-span-1 bg-gradient-to-br from-accent/10 to-background border border-accent/30 p-6 rounded-2xl h-fit">
          ...
          <div className="space-y-6">
            <div>
              ...
              <div className="text-3xl font-bold">{yearData.entryCount.toLocaleString()}<span className="text-lg font-normal text-slate-400 ml-1">名</span></div>
            </div>
